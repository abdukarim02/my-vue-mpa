    ...
    <div class="products-card__content container">
      ...
      <div class="products-card__info">
        ...
        <div class="products-card__info-price">
          <div v-if="priceOld" class="products-card__price-old">{{ formattedPriceOld }}</div>
          <div class="products-card__price-sale">{{ formattedPrice }}</div>
        ...
        <div class="products-card__button">
          <div class="products-card__button-counter">
            <button :disabled="quantity === 1" @click="decreaseQuantity">
...
const image = route.query.image || "No img";
...
const shortText = computed(() => {
  if (!text) return "";
  return isExpanded.value || text.length <= TEXT_LIMIT ? text : text.slice(0, TEXT_LIMIT) + "...";
});

const showReadMoreButton = computed(() => text.length > TEXT_LIMIT);

const formattedPrice = computed(() => price ? `${price.toFixed(2)}$` : "");
const formattedPriceOld = computed(() => priceOld ? `${priceOld.toFixed(2)}$` : "");

const addToCart = () => {
  emit("add-to-cart", { image, quantity: quantity.value, price });
