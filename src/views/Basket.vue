<template>
  <div class="basket">
    <div class="basket__content container">
      ...
      <div class="basket__content-body" v-if="cartItems.length">
        <div class="basket__body-card">
          <div v-for="item in cartItems" :key="item.id" class="basket__card">
              ...
              <div class="basket__card-info">
                ...
                <div class="basket__info-footer">
                  ...
                  <div class="basket__footer-price">
                    <span class="basket__price-old" v-if="item.oldPrice">${{ item.oldPrice }}</span>
        ...
        <div class="basket__body-order">
          ...
          <form class="basket__order-form" @submit.prevent="submitOrder">
            <input type="text" v-model="customerName" class="basket__form-input" placeholder="Name" required pattern="^[A-Za-zА-Яа-яЁё\s]+$" title="Only letters are allowed" />
            <input type="tel" v-model="customerPhone" class="basket__form-input" placeholder="Phone number" required pattern="^\+\d{11}$" title="Phone number must start with '+' and contain 11 digits" />
            <input type="email" v-model="customerEmail" class="basket__form-input" placeholder="Email" required />
...
const getCartFromLocalStorage = () => {
  ...
  return storedCart ? JSON.parse(storedCart) : [
    ...
    // { id: 2, name: "Pruner", image: "/src/assets/product-2.jpg", price: 200, oldPrice: 180, quantity: 1 },
  ];
};

const cartItems = ref(getCartFromLocalStorage());
...
// Слежение за изменениями корзины
watch(cartItems, updateLocalStorage, { deep: true });
...
const submitOrder = () => {
  if (customerName.value && customerPhone.value && customerEmail.value.includes("@")) {
